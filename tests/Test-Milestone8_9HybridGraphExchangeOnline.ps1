...
Assert-ContainsText $ui 'Initialize-HybridRuntime -ProfilePath $script:SelectedRuntimeProfileSummary.Path -RootPath $repoRoot -Force' 'Runtime launch forces the selected profile bootstrap instead of reusing startup simulation runtime'
Assert-ContainsText $ui 'Assert-HapRuntimeLaunchReadiness -Runtime $script:HybridRuntime' 'Runtime launch validates delegated Graph bootstrap completion before opening the dashboard'
Assert-ContainsText $ui "if (`$runtimeMode -eq 'Simulation' -or `$graphMode -eq 'Simulation') { return }" 'Runtime launch does not require delegated Graph sign-in for simulation profiles'
Assert-True (-not ($ui -match 'Invoke-HapLaunchGraphDelegatedPrompt')) 'Runtime launch does not use the duplicate UI delegated-auth probe'
Assert-True (-not ($ui -match 'Get-HybridRuntimeDisplayValue -InputObject \$graphRecord -Names @\(''Service''\)')) 'Runtime launch does not stringify the Graph service before authentication'
