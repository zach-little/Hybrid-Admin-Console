...
Assert-True ($ui -match 'ConvertTo-HapNewUserOptionArray' -and $ui -match '\[AllowNull\(\)\]\[object\]\$Items') 'New User Wizard dropdown loader accepts raw JSON arrays and single values'
Assert-True ($ui -match 'NewUserWizardComboBox' -and $ui -match 'Add-HapComboBoxTextItem' -and $ui -match '-- Select home organization --' -and $ui -notmatch 'ObservableCollection\[object\]') 'New User Wizard dropdown loader renders visible string items with placeholders instead of blank object-bound rows'
Assert-True ($ui -match '\$selected -is \[string\]' -and $ui -match "PSObject.Properties.Name -contains 'Value'" -and $ui -match "PSObject.Properties.Name -contains 'Display'" -and $ui -match 'Select\|Optional' -and $ui -match 'No values configured') 'New User Wizard selection reader handles string, placeholder, and object-bound dropdown values'
$newUserOpenBlock = $ui.Substring($ui.IndexOf('function Show-HybridNewUserWizardView'), $ui.IndexOf('function Show-HybridDeviceManagementView') - $ui.IndexOf('function Show-HybridNewUserWizardView'))
Assert-True ($newUserOpenBlock.IndexOf('Reset-HybridNewUserWizardFields -PreserveDefaults') -lt $newUserOpenBlock.IndexOf('Update-HapNewUserWizardConfigurationFromConfig')) 'New User Wizard resets fields before loading config-backed dropdown values'
...
$newUserConfigurationBlock = $ui.Substring($ui.IndexOf('function Get-HapNewUserWizardConfiguration'), $ui.IndexOf('function Format-HapNewUserOption') - $ui.IndexOf('function Get-HapNewUserWizardConfiguration'))
Assert-True ($newUserConfigurationBlock.IndexOf('$runtimeSettings = Get-HapProfileObjectValue') -lt $newUserConfigurationBlock.LastIndexOf('foreach ($configObject in @($profileConfig))')) 'Organization config.json is applied after runtime profile NewUserWizard defaults'
...
Assert-True ($graphProvider -match 'Resolve-HybridMicrosoftGraphUserRequestIdentity' -and $graphProvider -match 'onPremisesSamAccountName' -and $graphProvider -match "startswith\(userPrincipalName") 'Graph provider resolves mailboxless ADM short names through Entra identity filters'
