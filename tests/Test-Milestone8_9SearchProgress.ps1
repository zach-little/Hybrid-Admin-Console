...
Assert-Contains -Content $ui -Expected 'function Invoke-HybridHydrationStageQueueTick' -Message 'Hydration queue has stage pump handler'
Assert-Contains -Content $ui -Expected 'Invoke-HybridHydrationStageQueueTick' -Message 'Hydration queue directly invokes the stage pump'
Assert-Contains -Content $ui -Expected '$script:HybridHydrationQueue.Count -gt 0' -Message 'Hydration queue drains all remaining card stages'
Assert-Contains -Content $ui -Expected 'DoEvents()' -Message 'Hydration stages yield UI events between cards'
