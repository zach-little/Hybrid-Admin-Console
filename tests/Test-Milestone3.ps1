...
Assert-True ($null -eq $plainAlex.ManagerUser) 'Plain user is not manager-hydrated'
...
Assert-True ($null -ne $hydratedAlex.ManagerUser) 'Hydrated user has manager'
Assert-True ($hydratedAlex.DirectReports.Count -ge 1) 'Hydrated user has direct reports'
...
Assert-True ($overview.DirectReportCount -eq $hydratedAlex.DirectReports.Count) 'Overview direct report count matches hydration'
...
Assert-True (($overview.Cards | Where-Object { $_.Name -eq 'Manager' } | Measure-Object).Count -eq 1) 'Overview contains manager card'
Assert-True (($overview.Cards | Where-Object { $_.Name -eq 'DirectReports' } | Measure-Object).Count -eq 1) 'Overview contains direct reports card'
