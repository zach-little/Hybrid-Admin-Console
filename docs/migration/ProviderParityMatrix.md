...
| MicrosoftGraph | Auth/Health/Reads | Graph profile, auth posture, device/license/PIM reads | `Core.Provider.MicrosoftGraph.psm1` and graph modules | `HILOP.Providers.Graph.MicrosoftGraphProvider` | Foundation implemented | Live calls and writes are gated; no PowerShell dependency in native provider. |
| ActiveDirectory | Connection/Reads/Writes | AD user/group/manager/admin workflows | `Infrastructure.ActiveDirectory.psm1` | `HILOP.Providers.ActiveDirectory.ActiveDirectoryProvider` | Validated | Native AD writes passed non-production lab validation; execution remains constrained by the technician's effective AD permissions and local safety confirmations. |
| ExchangeOnline | Auth/LimitedReads | Mailbox and recipient administration | `Core.Provider.ExchangeOnline.psm1` | `HILOP.Providers.ExchangeOnline.ExchangeOnlineProvider` | Supportability gated | Unsupported admin operations return explicit unsupported results. |
...
| ExchangeOnPremises | Health/LimitedReads/Writes | remote mailbox and DG administration | `Infrastructure.ExchangeOnPremises.psm1` | `HILOP.Providers.ExchangeOnPremises.ExchangeOnPremisesProvider` | Supportability gated | Built-in provider does not use EMS, remote PowerShell, or direct Exchange-owned AD edits. |
...
| Runtime | Legacy migration infrastructure | temporary worker/bridge/protocol/adapter | `HILOP.Providers.LegacyPowerShell`, `HILOP.LegacyWorker.Protocol`, `HILOP.LegacyPowerShellWorker`, `HILOP.LegacyBridge.psm1` | Removed | Complete | Removed from solution/source after full build and test validation. |
...
- `src-dotnet/src/HILOP.Providers.Abstractions/IProviderHealthCapability.cs`
- `src-dotnet/src/HILOP.Providers.Abstractions/IUserLookupCapability.cs`
- `src-dotnet/src/HILOP.Providers.Abstractions/UserLookupModels.cs`
- `src-dotnet/src/HILOP.Providers.Simulator/DirectorySimulatorProvider.cs`
- `src-dotnet/src/HILOP.Providers.Simulator/DirectorySimulatorOptions.cs`
- `src-dotnet/src/HILOP.Providers.Simulator/DirectorySimulatorSeedData.cs`

Test files:

- `src-dotnet/tests/HILOP.Providers.Simulator.Tests/HILOP.Providers.Simulator.Tests.csproj`
- `src-dotnet/tests/HILOP.Providers.Simulator.Tests/DirectorySimulatorProviderTests.cs`
...
- Do not call `pwsh.exe`, `powershell.exe`, `System.Management.Automation`, the legacy worker, the legacy bridge, or the plugin host from `HILOP.Providers.Simulator`.
