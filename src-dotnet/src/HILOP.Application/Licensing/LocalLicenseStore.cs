...
public interface ILocalLicenseStore
{
    ...
    Task<string> RotateInstallationIdAsync(CancellationToken cancellationToken = default);
    ...
    Task ClearLicenseEnvelopeAsync(CancellationToken cancellationToken = default);
...
public sealed class FileLocalLicenseStore : ILocalLicenseStore
{
    ...
    public async Task<string> RotateInstallationIdAsync(CancellationToken cancellationToken = default)
    {
        Directory.CreateDirectory(_storageDirectory);
        var installationId = $"hilop-{Guid.NewGuid():N}";
        await File.WriteAllTextAsync(InstallationIdPath, installationId, cancellationToken).ConfigureAwait(false);
        return installationId;
    }
