...
public sealed class LicensingService : ILicensingService
{
    ...
    public async Task<OperationResult<LicensingStatus>> DeactivateAsync(CorrelationId correlationId, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(credential))
        {
            return LicensingErrors.Failure<LicensingStatus>(correlationId, "Licensing.CredentialMissing", "No installation credential is available.");
        }
        ...
        _auditSink.Record("INSTALLATION_DEACTIVATED", new Dictionary<string, string> { ["installation_id"] = installationId });
        return OperationResult<LicensingStatus>.Success(
            new LicensingStatus
            {
                State = LicenseState.Unlicensed,
                InstallationId = installationId,
                Message = "Installation deactivated. Existing configuration was preserved."
            },
            correlationId,
            status: "Deactivated");
    }
