...
using HILOP.Extensions.Registry;
using HILOP.Plugin.Protocol;
using Xunit;

namespace HILOP.Extensions.PowerShell.Tests;
...
public sealed class PowerShellProviderProxyTests
{
    ...
    public async Task InvokeOperationAsync_DoesNotLaunchHostWhenProviderDisabled()
    {
        ...
        {
            PluginHostPath = @"Z:\missing\HILOP.PowerShellPluginHost.dll"
        });
    ...
    public async Task InvokeOperationAsync_StartsHostForEnabledProvider()
    {
        ...
        var hostPath = Path.Combine(solutionRoot, "src", "HILOP.PowerShellPluginHost", "bin", "Debug", "net10.0", "HILOP.PowerShellPluginHost.dll");
        using var workspace = TempWorkspace.Create();
